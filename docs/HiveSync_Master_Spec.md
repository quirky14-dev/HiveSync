...
- **HiveSync: Request AI Documentation (First 500 Lines)**
...
### Free Tier Behavior
- Only first 500 lines are submitted.
...
If Free-tier:
- On the first click in a session of the "Apply Changes" button, Show modal with message:
  > “Upgrade to Pro in the mobile app to Generate/Save full AI Documentation”
- After the first click, message no longer appears when "Apply Changes" button is clicked
...
### 1.5 iPad Workflow: Desktop → iPad → Phone Preview

#### Desktop Edits → Worker → Sync
- Developer edits a mobile app project on the desktop client.
- Worker generates patch suggestions (comment blocks, code diffs, etc.).
- Patches and file updates are synced to all devices via the existing sync layer:
  - Desktop client
  - iPad app (React Native)
  - Mobile phone app (React Native)

#### iPad Review & Light Editing
When the user opens the project on iPad:

- The iPad app shows:
  - Project file tree
  - Pending patches / diffs
  - Status (applied, pending, rejected)

- The user can:
  - Review diffs
  - Approve or deny suggested blocks
  - Drag-to-reorder comment blocks within a file
  - Make small edits to code (e.g., fix typos, adjust numeric UI values, tweak strings)
  - Save changes, which sync back through the backend to the source of truth (repo / workspace).

#### Send to Phone for Preview
From the iPad, for **mobile app projects only**:

- The user can tap **“Send to Phone for Preview”**.
- The iPad sends the current file(s) / project state to the backend preview service.
- The backend:
  - Builds a lightweight React Native preview bundle for the mobile app project.
  - Targets one or more registered phone devices for this user.
- The phone app receives the new preview bundle and:
  - Automatically refreshes its preview mode.
  - Shows the latest build corresponding to iPad changes.

> Constraint: **Only mobile app projects** (React Native projects) are eligible for “Send to Phone for Preview”. Non-mobile projects are ignored or show the action as disabled.

#### Iterative Tweaks
When the user tweaks code again on iPad:

- iPad → sends updated files (or patch delta) to backend.
- Backend rebuilds the preview bundle (incremental / cache-aware where possible).
- Phone app auto-refreshes again with the latest preview state.

This allows a tight loop:
**Desktop edits → AI patches → iPad review → Phone live preview → back to iPad tweaks**, without requiring the user to rebuild manually on the desktop.


---

### 1.6 React Native Mobile Clients

- Both the iPad and phone apps are built in React Native (shared codebase, platform and form-factor specific layouts).
- Backend remains language-agnostic; examples assume a Node/TypeScript or similar API layer but are not strictly tied to it.

### Preview Pipeline for Mobile React Native Projects

#### 1) Device Registration

Endpoint (example):

- `POST /api/preview/devices/register`
  - Body: { userId, deviceId, platform, deviceType (phone | tablet), capabilities }
  - Purpose: register a preview-capable React Native device for a user.

#### 2) Send to Phone for Preview (from iPad or Desktop)
...
- `POST /api/preview/send`
  - Body:
    ...
    - projectId
    - source: "ipad" | "desktop"
    - optional: targetDeviceId (otherwise all preview devices for user)
  - Server behavior:
    - Validate that project type is **mobile app (React Native)**.
    - Pull latest project state (including iPad edits if applicable).
    - Invoke preview builder.

#### 3) Preview Builder Service

- Takes projectId + branch/commit/context + user-scoped changes (desktop + iPad).
- Produces a **React Native preview bundle**:
  - Compatible with current phone runtime (RN version, platform).
  - Prefer incremental builds using caching to keep turnaround fast.
- Publishes bundle metadata into a preview registry:
  - { userId, projectId, bundleUrl, createdAt, RN version }

#### 4) Notification to Devices

- Once a new bundle is ready:
  - Backend notifies registered devices for that user (e.g., via push or WebSocket).
  - Payload includes:
    - projectId
    - bundleUrl
    - version / hash
- Devices decide whether to download and apply (phone app does this automatically in preview mode).

#### 5) Security & Auth

- All preview-related endpoints require:
  - Authenticated user tokens.
  - Device registration checks (device must belong to the same user).
- Bundles are:
  - Scoped per user and project.
  - Time-limited (signed URLs or similar).

### Sync with Patch/Worker System

- Worker that generates patches/diffs for desktop also feeds into:
  - iPad diff views
  - Block suggestions (with block IDs and ordering)
- When user reorders or approves/denies blocks on iPad:
  - iPad sends updated block state back to backend.
  - Backend:
    - Applies changes to source of truth.
    - Regenerates or updates diffs as needed.
    - Ensures preview builder uses the latest state when building mobile app previews.
...
- AI documentation limited to **first 500 lines per file**
...
- **Full Support for Github Branching and Branch Switching**
- Priority queue for AI jobs
- Inline metadata enhancements
- Project pinning
- Increased storage (5 GB+ per user)
- Ability to save full AI-generated documentation back to GitHub
...
Free tier users may submit up to **the first 500 lines** of any file for AI
documentation. AI-generated comments beyond line 500 are not stored, not applied
...
If a Free user manually edits or rewrites comments beyond line 500 (thus making
it their own user-written content), HiveSync treats this as normal user code and
does not block or remove it.

Submitting an entire file (from the context menu in an OS for example) will result
in the **backend ONLY reading the first 500 lines** of that file

A free-tier user should be notified once per session when submitting code for documentation
that only the first 500 lines are submitted, inviting them to upgrade to Pro in the mobile app to remove this limitation.
- no AI generated output will be added below line 500.
...
HiveSync’s Free tier supports **full GitHub push + pull** capabilities,
allowing users to pull existing GitHub workflows into Hivesynce's space,
syncing it automatically to all devices connected to the users account,
and pushing modified files to GitHub.


Pro tier expands GitHub functionality with:
- Pushing to GitHub repositories automatically when updated locally, when enabled in user settings
- Extended diff metadata
- Project-level snapshot integration with Git history
- Branching and Branch Switching, and should store files in parent folders signifying which branch it is. 
...
single-file variable suggestions that don't break project functionality.
...
transformations that operate across the entire project, subject to user approval.
...
---
## New Global Behavioral Guarantees
- Cached AI metadata used when code is unchanged.
- Force regeneration available for specific regions.
